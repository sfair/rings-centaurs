...
//-------------------Box---------------------------//
   ...
   // reb_mpi_init(r);
    ...
    //------------------------Ring Particles-----------//
       ...
       int N = (int)(tau *r->boxsize.x*r->boxsize.y / (M_PI * particle_radius * particle_radius));
...
void heartbeat(struct reb_simulation* const r){
  if(reb_simulation_output_check(r,2.*M_PI/r->ri_sei.OMEGA)){   
     ...
     sprintf(parameters_filename, "parameters_cf0.5.txt");
    ...
    fprintf(satellite, "%e\t%e\t%e\t%e\t%e\t%e\n", 
     ...
     char filename[50];
     sprintf(filename, "positions_T%d__cf0.5.aei", period_count);

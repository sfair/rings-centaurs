...
//-------------------Box---------------------------//
    ...
    reb_simulation_configure_box(r, boxsize, 2, 7, 1);
    printf("Boxsize_y = %.2f m\n", r->boxsize.y);
    printf("Boxsize_x = %.2f m\n", r->boxsize.x);
    //---------------------MPI-----------------------//
    ...
    r->minimum_collision_velocity = particle_radius*OMEGA*0.001; 
    ...
    //------------------------Ring Particles-----------//
       ...
       int N = (int)(tau *r->boxsize.x*r->boxsize.y / (M_PI * particle_radius * particle_radius));/// r->mpi_num);
       ...
       for (int i=0; i<N; i++) {
           ...
           double        particle_mass = particle_density*4./3.*M_PI*particle_radius*particle_radius*particle_radius;
           pt.m         = particle_mass;//0;    // kg
           //pt.hash = i+1; 
...
// Output
void heartbeat(struct reb_simulation* const r){
  if(reb_simulation_output_check(r,(2.*M_PI/r->ri_sei.OMEGA)/5.)){   
    
     static int period_count = 0;

     char parameters_filename[50];
     sprintf(parameters_filename, "parameters_cf0.5.txt");//, r->mpi_id);
    ...
    struct reb_particle* sat_ptr = reb_simulation_particle_by_hash(r, SAT_HASH);
    
    FILE* satellite = fopen("satellite.txt","a");
    fprintf(satellite, "%e\t%e\t%e\t%e\t%e\t%e\n", 
     ...
     char filename[50];
     sprintf(filename, "positions_T%d_cf0.5.aei", period_count); //r->mpi_id);
    ...
    period_count++;
    printf("Número de partículas restantes: %d\n", r->N);

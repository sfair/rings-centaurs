...
//---------------------------------------------------------------------------------------------------------------------------//
    ...
    r->ri_sei.OMEGA       = OMEGA                                        ;       //frequência epíciclica          
    r->G                  = 6.67428e-11                                  ;         
...
//-----------------------------------------------//
    ...
    double particle_radius = lambda_toomre/(48*M_PI*tau*rh*rh*rh);
    printf("Particle radius: %.2f\n",particle_radius );
...
//-------------------------------------------//
       ...
       for (int i=0; i<N; i++) {
           ...
           pt.ay         = 0;
           pt.az         = 0;
           double radius     = particle_radius;//reb_random_powerlaw(r, particle_radius_min,particle_radius_max,particle_radius_slope);
           pt.r         = radius;                        // m
...
double gravitational_viscosity(const struct reb_simulation* const r){
    ...
    // Soma dos torques gravitacionais entre pares (i<j)
    for (int i = 0; i < r->N; i++){
        Mtotal += r->particles[i].m;
...
void heartbeat(struct reb_simulation* const r){
  if(reb_simulation_output_check(r,2.*M_PI/r->ri_sei.OMEGA)){   
     ...
     fprintf(parameters, "%e\t%e\t%e\t%e\t%e\t%e\t%e\t%e\t%e\t%e\n", 
            ...
            collis_visc,
            grav_visc,
            transl_visc+collis_visc+grav_visc);  // ou escreva separados se quiser

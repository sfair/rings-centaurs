...
//-------------------------------------------------------------------------------------------------------------
    ...
    reb_simulation_integrate(r,4*2.*M_PI/OMEGA);
...
// Save the positions of the particles in a file
...
void heartbeat(struct reb_simulation* const r){
  if(reb_simulation_output_check(r,2.*M_PI/r->ri_sei.OMEGA)){   
     ...
     //struct reb_particle* sat_ptr = reb_simulation_particle_by_hash(r,0);
     //FILE* sattelite = fopen("sattelite.txt","a");
     //fprintf(sattelite, "%e\t%e\t%e\t%e\t%e\t%e\n", r->t,sat_ptr->x,sat_ptr->y,sat_ptr->z,sat_ptr->r,sat_ptr->m);
     //fclose(sattelite);
     if (sat_index >= 0 && sat_index < r->N) {
      struct reb_particle* sat = r->particles;
      FILE* f = fopen("satelite.txt", "a");
      fprintf(f, "%e\t%e\t%e\t%e\t%e\t%e\n", r->t,
          ...
          fprintf(stderr, "ERRO: sat_index inválido: %d (r->N=%d)\n", sat_index, r->N);
      }
